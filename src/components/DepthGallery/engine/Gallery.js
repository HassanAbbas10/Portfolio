...
export class Gallery {
  constructor() {
    ...
    this.planeFadeSmoothing = 0.07

    this.parallaxEnabled = true
    this.parallaxAmountX = 0.16
    this.parallaxAmountY = 0.08
    this.parallaxSmoothing = 0.05
    ...
    this.gestureParallaxSmoothing = 0.035
    this.driftCurrent = 0
    this.driftTarget = 0
  ...
  setPlanes(scene) {
    ...
    this.planeConfig.forEach((plane, index) => {
      const texture = this.texturesBySource.get(plane.name) || null
  ...
  getPlaneLabelData(planeDefinition, index) {
    const fallback = {
      word: planeDefinition.name || `tech ${String(index + 1).padStart(2, '0')}`,
      category: planeDefinition.category || '',
      color: '',
    }
    ...
    return {
      ...
      category: label.category || fallback.category,
      color: label.color || fallback.color,
    }
  }

  updatePlaneScale() {
    const isMobileViewport = this.isMobile()
  ...
  isMobile() {
    return (typeof window !== 'undefined' ? window.innerWidth : 1024) <= this.mobileBreakpoint
  }
  ...
  getXSpreadFactor() {
    return this.isMobile() ? this.mobileXSpreadFactor : 1
  }
  ...
  setPreloadedTextures(texturesByKey) {
    this.texturesBySource = texturesByKey instanceof Map ? texturesByKey : new Map()
  }
  ...
  updatePlaneMotion(scroll = null) {
    ...
    this.planes.forEach((plane, index) => {
      ...
      const aspectRatio = plane.userData.aspectRatio || 1
      const baseScale = this.isMobile() ? this.mobilePlaneScale : this.desktopPlaneScale
