    ...
    <>
      <div className="bg-gradient-to-br from-gray-950 via-black to-gray-950 pt-10">
        ...
        >
          ...
          {/* Filter Buttons */}
          <motion.div className="flex justify-center gap-4 mt-8 mb-8" variants={containerVariants}>
            {availableFilters.map((filter) => (
              <motion.button
                key={filter}
                className={`px-6 py-2 rounded-full font-medium transition-all duration-300 ${
                  activeFilter === filter ? "bg-custom-green text-black" : "bg-gray-800 text-white hover:bg-gray-700"
                }`}
                ...
                {filteredProjects.map((proj) => (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-center">
                      <div className="mt-4 text-xl font-bold font-quicksand text-custom-green">{proj.heading}</div>
                    </div>

                    {/* Project Description */}
                    <div className="flex items-center justify-center">
                      <p className="max-w-sm mt-2 text-white text-md font-cormorant">{proj.description}</p>
                    </div>
                      ...
                      <div className="max-w-sm p-2 mt-1 text-sm text-black rounded-xl dark:text-neutral-300">
                        <span
                          className={`px-3 py-1 rounded-full text-sm font-medium ${
                            proj.status === "Completed" ? "bg-green-900 text-green-400" : "bg-yellow-900 text-yellow-400"
                          }`}
                        >
                          {proj.status}
                        </span>
